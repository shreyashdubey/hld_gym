...
function blank() {
  return {
    ...
    pos: {},        // chapterId -> scrollY where reading stopped
    lastCh: null,
  };
}
let S = blank();
...
function renderRail(id, readMin) {
  ...
  railSpy = () => {
    ...
    requestAnimationFrame(() => {
      ...
      // the bookmark: saved on a debounce so the scroll loop never hits storage
      S.pos[id] = Math.round(scrollY);
      clearTimeout(posT); posT = setTimeout(save, 800);
    });
  };
...
function renderChapter(id) {
  ...
  const back = S.pos[id] || 0;
  window.scrollTo(0, back > innerHeight ? back : 0);
  if (back > innerHeight) toast('picked up where you left off');
