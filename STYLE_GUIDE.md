...
| year agreement | a year inside a `<span class="fact" … data-cite="key">` must equal that source's `year`. Attribute order does not matter, the class is matched inside a multi-class attribute, and the tag is matched case-insensitively — this is the **same** span matcher the cited-fact check uses, so a span the build counts as your citation is always a span whose year it reads |
| unused source | a source nothing cites is a **warning** |

**What it does not check, and you therefore must.** Three of the nine rules have no validator at all, one has a weaker one than it looks, one field is unchecked, and two gaps are open by choice. This list is the honest inventory — a gap we decided to live with belongs here just as much as one nobody has got to:

- **O5** is per-fact; the build's gate is per-chapter. One `data-cite` on one `.fact` span satisfies it, so a story with nineteen uncited dates and one cited one passes. Nothing tells you which date you left uncited. The year-agreement check likewise fires only on a `.fact` span whose `data-cite` holds a single key — put two space-separated keys on a fact and its year goes unchecked. What the gate *no longer* accepts: card `cite` keys, which are excluded from both it and the primary-source check.
- **An empty fact span satisfies the cited-fact check.** `<span class="fact" data-cite="k"></span>` has the class and the key and no content, and the build takes it. This gap is deliberate and was left open with the reviewer's agreement: requiring text inside the span would break the legitimate cases of citing an `<img>` or a nested `<code>`, and exploiting it means writing a visibly pointless element that a human reviewer would query on sight. It is disclosed here rather than closed, so nobody discovers it and assumes it was missed. Do not use it.
- **An uppercase `DATA-CITE=` is invisible to every citation check.** HTML would honour it; the build's three scans all match `data-cite` lowercase. It fails safe — your chapter errors with "carries no data-cite" rather than passing — but the message will not match what you think you see in the file. Write the attribute lowercase.
